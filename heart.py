...
async def process_love_words(event: NewMessage.Event):
    ...
    await asyncio.sleep(1)
    await client.edit_message(event.peer_id.user_id, event.message.id, 'я люблю когда')
    await asyncio.sleep(1)
    await client.edit_message(event.peer_id.user_id, event.message.id, 'я люблю когда волосатые')
    await asyncio.sleep(1)
    await client.edit_message(event.peer_id.user_id, event.message.id, 'я люблю когда волосатые')
        await asyncio.sleep(1)
    await client.edit_message(event.peer_id.user_id, event.message.id, 'я люблю когда волосатые мужики')
        await asyncio.sleep(1)
    await client.edit_message(event.peer_id.user_id, event.message.id, 'я люблю когда волосатые мужики обмазываются')
        await asyncio.sleep(1)
    await client.edit_message(event.peer_id.user_id, event.message.id, 'я люблю когда волосатые мужики обмазываются маслом❤🧡💛💚💙💜🤍💖')
